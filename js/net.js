...
// 매치 채널 개설 + Presence 등록 (자동매칭/친선전 공용)
async function openMatchSession(match, isHost, handlers, myId) {
  ...
  ch.on('broadcast', { event: 'attack' }, ({ payload }) => handlers.onOpponentAttack?.(payload));
...
export async function findMatch(handlers) {
  ...
  // 1) 대기 중인 매치 참가 시도 (내가 만든 방·친선전 방 제외)
  ...
  return openMatchSession(match, isHost, handlers, myId);
}

// ── 친선전: 초대 코드로 방 생성 ──
const CODE_CHARS = 'ABCDEFGHJKMNPQRSTUVWXYZ23456789'; // 혼동 문자(I/L/O/0/1) 제외
export async function createFriendMatch(handlers) {
  if (!sb) return null;
  const myId = await uid();
  ...
  const { data: ins, error } = await sb
    .from('matches')
    .insert({ p1: myId, seed, status: 'waiting', code })
    .select();
  if (error) { console.error(error.message); return null; }
  return openMatchSession(ins[0], true, handlers, myId);
}

// ── 친선전: 코드로 참가 ──
export async function joinFriendMatch(code, handlers) {
  if (!sb) return null;
  const myId = await uid();
  const { data: found } = await sb
    .from('matches')
    .select('id, seed, p1, code')
    .eq('status', 'waiting')
    .eq('code', code.toUpperCase().trim())
    .limit(1);
  if (!found || found.length === 0) return null; // 코드 없음/이미 시작됨
  if (found[0].p1 === myId) return null;          // 자기 방 참가 방지
  const { data: upd } = await sb
    .from('matches')
    .update({ status: 'playing', p2: myId })
    .eq('id', found[0].id)
  ...
  return openMatchSession(upd[0], false, handlers, myId);
}
