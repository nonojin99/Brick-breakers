...
export class Engine {
  constructor(config = CONFIG) {
    ...
    this.pendingAttacks = 0;                      // 상대에게 받은 공격 줄 (턴 종료 시 적용)
  }
  ...
  // ── 공격 시스템 (대전용) ──
  // 상대의 공격을 큐에 적재. 비행 중 그리드 변형 방지를 위해 endTurn에서 일괄 적용
  queueAttack(rows) {
    this.pendingAttacks += rows;
  }

  applyAttacks() {
    while (this.pendingAttacks > 0 && !this.gameOver) {
      this.pendingAttacks--;
      this.descend(); // 전체 1칸 밀어내림 + 게임오버 판정
      if (this.gameOver) return;
      // 공격 줄: 구멍 2개, HP는 현재 라운드의 절반 (최소 1)
      const row = Array(this.cfg.cols).fill(null);
      const holes = new Set();
      while (holes.size < 2) holes.add(Math.floor(this.rng() * this.cfg.cols));
      const hp = Math.max(1, Math.ceil(this.round / 2));
      for (let c = 0; c < this.cfg.cols; c++) {
        if (!holes.has(c)) row[c] = { type: 'attack', hp };
      }
      this.grid[0] = row;
    }
  }

  // 턴 종료 처리: 하강 → 게임오버 판정 → 신규 행 → 공격 적용 → 발사 위치 갱신 → round++
  endTurn() {
